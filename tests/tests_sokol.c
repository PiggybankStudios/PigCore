...
void AppInit(void)
{
	...
	if (useActiveFont)
	{
		...
		MakeFontActive(&testFont, 64, 256, 8, 0, 0);
		attachResult = AttachOsTtfFileToFont(&testFont, StrLit(MAIN_FONT_NAME), 18*textScale, FontStyleFlag_None); Assert(attachResult == Result_Success);
		...
		// +==============================+
		{
			...
			#if 1
			{
				...
				Str8 displayStrs[] = {
					...
					StrLit("\xF0\x9F\x98\x8A\xF0\x9F\x98\x82\xF0\x9F\xA4\xA3\xF0\x9F\x98\x92\xF0\x9F\x98\x81[size=64]\xF0\x9F\x98\x8D\xF0\x9F\x99\x8C"), //😊😂🤣😒😁😍🙌
