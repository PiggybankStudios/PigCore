...
PEXP void DoUiTextbox(UiWidgetContext* context, UiTextbox* tbox, PigFont* font, u8 fontStyle, r32 fontSize)
{
	...
	// +==============================+
	if (context->mouse->isOverWindow && IsMouseBtnPressed(context->mouse, context->mouseHandling, MouseBtn_Left))
	{
	...
	// +==============================+
	if (tbox->isFocused && IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_Escape, false)) //TODO: Set handledUntilReleased
	{
	...
	if (tbox->isFocused)
	{
		if (IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_Left, true)) //TODO: Set handledUntilReleased
		{
			if (tbox->cursorActive)
			{
				...
				{
					if (IsKeyboardKeyDown(context->keyboard, nullptr, Key_CtrlOrCmd))
					{
		...
		if (IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_Right, true)) //TODO: Set handledUntilReleased
		{
			if (tbox->cursorActive)
			{
				if (tbox->cursorStart != tbox->cursorEnd && !IsKeyboardKeyDown(context->keyboard, nullptr, Key_Shift))
	...
	// |        Handle Ctrl+A         |
	// +==============================+
	if (tbox->isFocused && IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_A, true) && IsKeyboardKeyDown(context->keyboard, nullptr, Key_CtrlOrCmd)) //TODO: Set handledUntilReleased
	{
	...
	if (tbox->isFocused && tbox->cursorActive)
	{
		for (uxx iIndex = 0; iIndex < context->keyboard->numCharInputs; iIndex++)
		{
			...
			if ((context->keyboardHandling == nullptr || !context->keyboardHandling->charInputHandled[iIndex]) &&
				(charInput->modifierKeys == ModifierKey_None || charInput->modifierKeys == ModifierKey_Shift))
			{
				u8 utf8Bytes[UTF8_MAX_CHAR_SIZE];
				u8 codepointSize = GetUtf8BytesForCode(charInput->codepoint, &utf8Bytes[0], false);
				if (codepointSize > 0)
				{
					if (context->keyboardHandling != nullptr) { context->keyboardHandling->charInputHandled[iIndex] = true; }
	...
	if (tbox->isFocused && tbox->cursorActive)
	{
		if (tbox->cursorEnd != tbox->cursorStart &&
			(IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_Backspace, true) || IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_Delete, true)))
		{
			UiTextboxDeleteSelected(tbox);
		}
		else if (IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_Backspace, true) && tbox->cursorEnd > 0)
		{
			u8 prevCodepointSize = GetPrevCodepointForUtf8Str(tbox->text, tbox->cursorEnd, nullptr);
			if (prevCodepointSize == 0) { prevCodepointSize = 1; }
			UiTextboxDeleteBytes(tbox, tbox->cursorEnd-prevCodepointSize, prevCodepointSize);
		}
		else if (IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_Delete, true) && tbox->cursorEnd < tbox->text.length)
		{
	...
	// |      Handle Ctrl+C Copy      |
	// +==============================+
	if (tbox->isFocused && tbox->cursorActive && IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_C, true) && IsKeyboardKeyDown(context->keyboard, nullptr, Key_CtrlOrCmd) && context->windowHandle != OsWindowHandleEmpty)
	{
	...
	// |      Handle Ctrl+X Cut       |
	// +==============================+
	if (tbox->isFocused && tbox->cursorActive && IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_X, true) && IsKeyboardKeyDown(context->keyboard, nullptr, Key_CtrlOrCmd) && context->windowHandle != OsWindowHandleEmpty)
	{
		Str8 selectedText = StrSlice(tbox->text, MinUXX(tbox->cursorStart, tbox->cursorEnd), MaxUXX(tbox->cursorStart, tbox->cursorEnd));
		Result copyResult = OsSetClipboardString(context->windowHandle, selectedText);
	...
	// |     Handle Ctrl+V Paste      |
	// +==============================+
	if (tbox->isFocused && tbox->cursorActive && IsKeyboardKeyPressed(context->keyboard, context->keyboardHandling, Key_V, true) && IsKeyboardKeyDown(context->keyboard, nullptr, Key_CtrlOrCmd) && context->windowHandle != OsWindowHandleEmpty)
	{
