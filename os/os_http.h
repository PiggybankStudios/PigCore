...
static bool HttpRequestManagerStartRequest(HttpRequestManager* manager, uxx requestIndex)
{
	...
	Str8 verbStr = MakeStr8Nt(GetHttpVerbStr(request->args.verb));
	...
	#if TARGET_IS_WINDOWS
	{
		...
		else
		{
			...
			allHeaders[request->args.numHeaders + 0].value = MakeStr8Nt(GetMimeTypeOfficialName(request->args.contentEncoding));
