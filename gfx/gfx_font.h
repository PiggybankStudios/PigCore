...
PEXP FontAtlas* AddNewActiveAtlas(PigFont* font, FontFile* fontFile, r32 fontSize, u8 styleFlags)
{
	...
	newAtlas->glyphRange = MakeFontCharRangeSingle(0);
	...
	VarArrayLoop(&activeAtlas->charRanges, rIndex)
	{
		...
		if (removeGlyph->codepoint >= charRange->startCodepoint && removeGlyph->codepoint <= charRange->endCodepoint)
		{
			if (charRange->startCodepoint == charRange->endCodepoint && removeGlyph->codepoint == charRange->startCodepoint)
			{
				...
				{
					activeAtlas->glyphRange = MakeFontCharRangeSingle(0);
				}
				else if (rIndex == 0)
				{
...
PEXP FontGlyph* TryAddGlyphToActiveFontAtlas(PigFont* font, FontFile* fontFile, FontAtlas* activeAtlas, u32 codepoint, u8 extraStyleFlags)
{
	...
	if (foundSpace)
	{
		...
		newGlyph->metrics.renderOffset = MakeV2((r32)bitmapXOffset, (r32)bitmapYOffset); //TODO: Should we multiply this by fontScale?
		newGlyph->metrics.logicalRec = MakeRec(0, -activeAtlas->metrics.maxAscend, newGlyph->metrics.advanceX, activeAtlas->metrics.maxAscend);
		#endif //BUILD_WITH_FREETYPE
		...
		{
			activeAtlas->glyphRange = MakeFontCharRangeSingle(codepoint);
		}
		...
		if (glyphSize.Width > 0 && glyphSize.Height > 0)
		{
			...
			#else //!BUILD_WITH_FREETYPE
			{
				v2i bitmapSize = MakeV2i((i32)bitmapWidth, (i32)bitmapHeight);
				FontActiveAtlasTextureUpdate* newUpdate = VarArrayAdd(FontActiveAtlasTextureUpdate, &activeAtlas->pendingTextureUpdates);
				NotNull(newUpdate);
				ClearPointer(newUpdate);
				newUpdate->sourcePos = newGlyph->atlasSourcePos;
...
PEXP bool TryGetFontGlyphMetrics(PigFont* font, u32 codepoint, r32 fontSize, u8 styleFlags, FontGlyphMetrics* metricsOut)
{
	...
	#else //!BUILD_WITH_FREETYPE
	{
		...
		if (!IsCodepointWhitespace(codepoint, true))
		{
			...
			metricsOut->renderOffset = MakeV2(boxX0 * fontScale, boxY0 * fontScale);
		}
		int advanceX = 0;
...
PEXP Result TryBakeFontAtlasWithCustomGlyphs(PigFont* font, r32 fontSize, u8 styleFlags, i32 minAtlasSize, i32 maxAtlasSize, uxx numCharRanges, const FontCharRange* charRanges, uxx numCustomGlyphRanges, const CustomFontCharRange* customGlyphRanges)
{
	...
	do
	{
		...
		if (missingCodepoints.length > 0)
		{
			...
			else { newAtlas->glyphRange = MakeFontCharRangeSingle(0); }
		}
		...
		while (atlasSideLength <= maxAtlasSize)
		{
			...
			for (uxx rIndex = 0; rIndex < numCustomGlyphRanges; rIndex++)
			{
				...
				for (uxx gIndex = 0; gIndex < numGlyphsInCustomRange; gIndex++)
				{
					...
					reci sourceRec = AreEqual(customGlyph->sourceRec, (reci)Reci_Zero)
